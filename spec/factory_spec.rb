...
RSpec.describe MyFactory do

  before :each do
    @Foo = MyFactory.new(:one, :two)
    @bar = @Foo.new('a', 'b')
    @baz = @Foo.new('c', 'd')
  end

  it 'should have same ancestros as Struct' do
    expect(MyFactory.ancestors[1..-1]).to be == Struct.ancestors[1..-1]
  end

  it 'should have same superclass as Struct' do
    expect(MyFactory.superclass).to be == Struct.superclass
  end

  it 'it instances must have same ancestors' do
    expect(@Foo.ancestors[2..-1]).to be == Struct.new(:a).ancestors[2..-1]
  end
  ...
  it 'should properly create instances if first arg is String' do
    MyFactory.new('Foo', :bar)
    MyFactory::Foo.new('baz')
    expect { MyFactory.new('a', :b) }.to raise_error(NameError)
  end
